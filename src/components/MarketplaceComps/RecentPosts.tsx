...
function Recent() {
  return (
    <div className='w-[54%] py-8 flex items-center justify-center gap-4 flex-col'>
        <h1 className='w-full flex items-center justify-start py-8 text-2xl text-custom_blue font-bold'>Recently Added</h1>
        <div className='w-full flex items-center justify-between flex-wrap'>
          {nftsData && nftsData.length>0?nftsData.filter(data=>data.category=="recent").map((data:any,i:number)=>{
